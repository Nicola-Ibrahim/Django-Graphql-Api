...
# Make the dockerized-dgraph-run script executable
RUN chmod +x /backend/scripts/dockerized-dgraph-run.sh


# Run the dockerized-dgraph-run script as the default command when starting the container
ENTRYPOINT ["/backend/scripts/dockerized-dgraph-run.sh"]
